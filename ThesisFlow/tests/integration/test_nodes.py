...
@pytest.mark.parametrize("search_engine", [SearchEngine.ARXIV.value])
def test_background_investigation_node_arxiv(
    mock_state,
    ...
    """Test background_investigation_node with Arxiv search engine"""
...
def test_background_investigation_node_malformed_response(
    mock_state, mock_web_search_tool, patch_config_from_runnable_config, mock_config
):
    """Test background_investigation_node with malformed Arxiv response"""
    # Mock a malformed response
    mock_web_search_tool.return_value.invoke.return_value = "invalid response"
    ...
    # Parse and verify the JSON content
    results = result["background_investigation_results"]
    assert results == "invalid response"
